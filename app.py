...
def _post(path: str, payload: dict, timeout: int = 120):
    """Safe POST helper for the Financify backend."""
    try:
        ...
        raise RuntimeError(
            "Could not connect to Financify backend. Please try again in a few minutes."
        ) from exc
...
def check_subscription(email: str):
    ...
    try:
        ...
        return {"active": False, "error": "Could not verify subscription."}
...
def require_financify_access():
    """Shows login/paywall and stops the app until user has SaaS access."""
    st.sidebar.markdown("---")
    st.sidebar.subheader("🔐 Financify Access")

    if st.session_state.get("financify_logged_in"):
        email = st.session_state.get("financify_email", "")
        subscription = st.session_state.get("financify_subscription") or check_subscription(email)
        st.session_state["financify_subscription"] = subscription
        ...
        is_active = bool(
            ...
            or subscription.get("is_active")
            or subscription.get("subscribed")
            or subscription.get("status") in ["active", "trialing", "paid"]
        )
        ...
        <div class="hero">
            ...
            <h3>Premium Financify Tool</h3>
            <p>Your login is verified. Please activate your subscription to use this tool.</p>
        </div>
        """, unsafe_allow_html=True)

        st.warning("Your Financify subscription is not active for this account.")
        st.link_button("Activate Subscription", SURECART_CHECKOUT_URL)
        st.stop()

    st.markdown("""
    <div class="hero">
        <h1>🐝 Madness of Money Bees</h1>
        <h3>Financify SaaS Access Required</h3>
        <p>Login with your email to unlock the market cycle predictor.</p>
    </div>
    """, unsafe_allow_html=True)

    with st.container():
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.subheader("Login to Financify")

        email = st.text_input("Email", value=st.session_state.get("financify_email", ""))
        ...
        code = st.text_input("Login Code", type="password")
        ...
        with c2:
            if st.button("Verify & Continue", use_container_width=True):
                try:
                    ...
                    st.session_state["financify_email"] = email.strip().lower()
                    st.session_state["financify_user"] = result
                    st.session_state["financify_subscription"] = check_subscription(email)
                    st.rerun()
                except Exception as exc:
                    st.error(str(exc))

        st.info("Use the same email you used while buying Financify.")
        st.link_button("Buy / Renew Subscription", SURECART_CHECKOUT_URL)
        st.markdown('</div>', unsafe_allow_html=True)

    st.stop()


require_financify_access()
