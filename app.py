...
def calculate_trend_score(data):
    ...
    if len(nifty) < 220:
        return 50, {"Status": "Not enough Nifty data"}
    ...
    return clamp(score), {
        ...
        "Distance From 200DMA %": round(dist200, 2),
    }


def calculate_breadth_score(data):
    nifty = get_close(data.get("nifty"))
    bank = get_close(data.get("bank_nifty"))
    nifty_bees = get_close(data.get("nifty_bees"))

    markets = {
        "Nifty": nifty,
        "Bank Nifty": bank,
        "NiftyBees": nifty_bees,
    }

    usable = {k: v for k, v in markets.items() if len(v) > 220}

    if len(usable) < 2:
        return 50, {"Status": "Partial breadth proxy only"}
...
def calculate_liquidity_score(data):
    ...
    if len(volume) > 120:
        ...
        if vol120 > 0:
            ratio = vol20 / vol120
            score += 10 if ratio > 1 else -5
            details["Volume 20D / 120D"] = round(ratio, 2)
...
def calculate_valuation_score(data):
    ...
    if len(nifty) < 252:
        return 50, {"Status": "Valuation proxy only"}

    one_year_return = pct(nifty.iloc[-1], nifty.iloc[-252])
    one_year_high = nifty.rolling(252).max().iloc[-1]
    distance_from_high = pct(nifty.iloc[-1], one_year_high)
    ...
    return clamp(score), {
        ...
        "Note": "Automatic valuation proxy. Add PE/PB API later for better accuracy.",
    }
...
def calculate_sentiment_score(data):
    ...
    if len(vix) < 120:
        return 50, {"Status": "Not enough VIX data"}
...
def phase_rule_score(phase, raw):
    ...
    return clamp(base - penalty), base, penalty
...
def get_rule_results(raw):
    ...
    for phase in PHASES:
        final, base, penalty = phase_rule_score(phase, raw)
        rows.append({
            "Phase": phase,
            "Rule Score": final,
            "Base Score": base,
...
def xgb_predict(model, encoder, scores):
...
def apply_transition_filter(prob_df, previous_phase):
    ...
    for idx, row in adjusted.iterrows():
        phase_i = PHASES.index(row["Phase"])
        distance = min(abs(phase_i - prev_i), len(PHASES) - abs(phase_i - prev_i))
...
def hybrid_prediction(xgb_df, rule_rows):
    rule_df = pd.DataFrame(rule_rows)[["Phase", "Rule Score"]]
...
st.sidebar.caption("Automatic mode. Visitors do not need to enter scores.")
...
# LOAD MODEL
# =====================================================
model, encoder, model_features, missing_files = load_model()

if missing_files:
    st.error("Model files missing. Run `python train.py` first, then upload the generated .pkl files.")
...
try:
    ...
    st.error("Live market data failed.")
    st.exception(e)
    st.stop()

rule_rows = get_rule_results(live_scores)

xgb_df = xgb_predict(model, encoder, live_scores)
xgb_df = apply_transition_filter(xgb_df, previous_phase)
...
st.markdown(f"""
<div class="good">
<b>Live data loaded successfully</b><br>
...
Distribution appears near the top, where the index may still look strong but internal strength weakens.
Bear phases appear on the falling part. Accumulation appears near the bottom, where sentiment is weak
but long-term opportunity may begin.
</p>
</div>
""", unsafe_allow_html=True)
...
descriptions = {
    ...
    "Breadth": "Participation using Nifty, Bank Nifty, and NiftyBees proxies.",
    "Liquidity": "ETF trend, volume, USDINR pressure, and global yield pressure.",
...
for i, feature in enumerate(FEATURES):
    value = live_scores[feature]

    with cols[i % 3]:
        st.markdown(f"""
        <div class="card">
            <h3>{feature}</h3>
            <div class="big-score" style="color:{score_color(value)};">{value:.1f}/100</div>
            <p class="small">{descriptions[feature]}</p>
        </div>
        """, unsafe_allow_html=True)
...
if winner["Hybrid Score"] - runner["Hybrid Score"] < 7:
    ...
    This may mean the market is transitioning between phases.
    </p>
    </div>
    """, unsafe_allow_html=True)
...
<p>
This version is automatic and fast because XGBoost is trained outside Streamlit.
For the most accurate paid version, later connect official Nifty PE/PB, FII/DII flow,
PMI, CPI, GDP, and full advance-decline breadth data.
</p>
</div>
""", unsafe_allow_html=True)
