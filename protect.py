...
def get_current_access(tool_name: str, free_limit: int = 5, pro_limit: int = 100) -> dict:
...
def record_tool_use(tool_name: str, free_limit: int = 5, pro_limit: int = 100):
    email = st.session_state.get("financify_email", "")
    session_token = st.session_state.get("financify_token", "")
...
def login_ui(tool_display_name: str, free_limit: int = 5, pro_limit: int = 100):
    st.markdown(
        """
        <style>
        .fin-login-card {
    ...
    st.markdown(
        ...
        <div class="fin-login-card">
          ...
          <span class="fin-login-pill">{free_limit} free uses per month</span>
          <span class="fin-login-pill">{pro_limit} Pro uses per month</span>
        ...
        if st.button("Verify login", use_container_width=True, type="primary"):
            try:
                ...
                for key in list(st.session_state.keys()):
                    if key.endswith("_usage_recorded"):
    ...
    st.markdown(
        ...
        <div class="fin-upgrade-box">
          ...
          <div class="fin-upgrade-text">Upgrade once and unlock {pro_limit} uses per month for Financify tools.</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def require_tool_access(
    ...
    if not st.session_state.get("financify_logged_in"):
        login_ui(display_name, free_limit=free_limit, pro_limit=pro_limit)
        st.stop()

    try:
        access = get_current_access(tool_name, free_limit=free_limit, pro_limit=pro_limit)
    except Exception as e:
    ...
    plan = str(access.get("plan", "free") or "free").lower()
    default_limit = pro_limit if plan == "pro" else free_limit
    used = int(access.get("usage_count", access.get("used", 0)) or 0)
    limit = int(access.get("limit", default_limit) or default_limit)
    ...
    if not allowed:
        st.error(f"Your monthly limit is finished for this tool. Free users get {free_limit} uses/month.")
        st.write(f"Upgrade to Financify Pro for {pro_limit} uses/month.")
        upgrade_button()
        st.stop()
